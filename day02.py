...
def is_safe(raw_report):
    ...
    return (all(x < 0 for x in report) or all(x > 0 for x in report)) and all(abs(x) < 4 for x in report)
...
@timer
def part2():
    return sum(1 for report in data if any(is_safe(report[:idx] + report[idx+1:]) for idx in range(len(report))) or is_safe(report))
